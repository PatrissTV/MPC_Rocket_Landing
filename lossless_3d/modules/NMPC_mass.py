...
class MPC:
    def __init__(self):
        self.m0 = 100
        self.alpha = 0.1
        self.maxThrust = 15*self.m0
        self.minThrust = 3*self.m0
        self.finalRadius = 1
        self.finalZ = 0
    ...
    def model(self):
        ...
        self.m.mass = Var(self.m.t, initialize=self.m0)
        self.m.dmass = DerivativeVar(self.m.mass, wrt=self.m.t)

    def constraints(self):
        self.m.ode1 = Constraint(self.m.t, rule=lambda m, t: m.mass[t]*m.ddx1[t] == m.u1[t]*m.tf**2 - self.Cd*m.dx1[t]*abs(m.dx1[t]))
        self.m.ode2 = Constraint(self.m.t, rule=lambda m, t: m.mass[t]*m.ddx2[t] == m.u2[t]*m.tf**2 - self.Cd*m.dx2[t]*abs(m.dx2[t]))
        self.m.ode3 = Constraint(self.m.t, rule=lambda m, t: m.mass[t]*m.ddx3[t] == (m.u3[t] - 9.81*m.mass[t])*m.tf**2 - self.Cd*m.dx3[t]*abs(m.dx3[t]))

        self.m.thrust_gamma1 = Constraint(self.m.t, rule=lambda m, t: m.u1[t]**2 + m.u2[t]**2 + m.u3[t]**2 <= m.gamma[t]**2)
        self.m.mass_rate = Constraint(self.m.t, rule=lambda m, t: m.dmass[t] == -self.alpha*m.gamma[t]/(m.tf))
    ...
    def result(self):
        ...
        u1 = np.array([self.m.u1[t]() for t in self.m.t])/self.m0 
        u2 = np.array([self.m.u2[t]() for t in self.m.t])/self.m0 
        u3 = np.array([self.m.u3[t]() for t in self.m.t])/self.m0 
        ...
        gamma = np.array([self.m.gamma[t]() for t in self.m.t])/self.m0
        mass = np.array([self.m.mass[t]() for t in self.m.t])/self.m0
        self.mpc_nominal = [u1,u2,u3,x1,x2,x3,dx1,dx2,dx3,tt,gamma,mass]
    ...
    def nicePlot(self):
        ...
        plt.legend(['$T_1/\mathrm{m}_0$','$T_2/\mathrm{m}_0$','$T_3/\mathrm{m}_0$','$|T|/\mathrm{m}_0$'])

        plt.figure()
        plt.plot(self.mpc_nominal[9],self.mpc_nominal[10],color='b')
        plt.plot(self.mpc_nominal[9],np.sqrt(self.mpc_nominal[0]**2+self.mpc_nominal[1]**2+self.mpc_nominal[2]**2),color='black',linestyle='--',alpha=0.5)
        #horizontal line
        plt.fill_between(self.mpc_nominal[9],np.ones(len(self.mpc_nominal[9]))*self.minThrust/self.m0,np.ones(len(self.mpc_nominal[9]))*self.maxThrust/self.m0,color='grey',alpha=0.3)
        plt.xlabel('Time [s]')
        plt.ylabel('Thrust [m $\mathrm{s}^{-2}$]')
        plt.legend(['$\Gamma/\mathrm{m}_0$','$|T|/\mathrm{m}_0$','Allowed thrust'])

        plt.figure()
        plt.plot(self.mpc_nominal[9],self.mpc_nominal[11],color='b')
        plt.xlabel('Time [s]')
